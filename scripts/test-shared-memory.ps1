...
if (-not (Test-Path -LiteralPath $envFile)) {
    throw '.docker.env is required for the real shared-memory integration test.'
}
...
if ([string]::IsNullOrWhiteSpace($env:NEO4J_PASSWORD)) {
    throw 'NEO4J_PASSWORD is required for the real shared-memory integration test.'
}
if (-not $env:NEO4J_DATABASE) { $env:NEO4J_DATABASE = 'neo4j' }

$python = Join-Path $workspaceRoot '.venv\Scripts\python.exe'
& $python (Join-Path $PSScriptRoot 'shared_memory_integration.py')
exit $LASTEXITCODE
